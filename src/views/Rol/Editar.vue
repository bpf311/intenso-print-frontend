          ...
          <v-card-title>
            <v-row>
              ...
              <v-col cols="12" md="2" class="text-center text-md-end">
                <v-btn block color="error" :to="{ name: 'Listado de roles' }">
              ...
              <v-card class="fill-height elevation-10">
                ...
                <v-card-text>
                  <v-alert
                    ...
                    class="mb-8"
                  >
                    Por favor, revise y corrija cualquier error identificado a
                    continuación:
                    <ul>
                    ...
                    <v-col cols="12">
                      ...
                      >
                        <v-textarea
                          ...
                          color="blue darken-4"
                          :error-messages="
                            obtenerValidaciones(errors, 'descripcion')
                          "
                        />
                      </ValidationProvider>
                    </v-col>
              ...
              <v-card class="fill-height elevation-10">
                ...
                <v-card-text>
                  ...
                  <v-simple-table fixed-header :height="dynamicTableHeight">
                    <tbody
                      v-for="(
                        permisosCategoria, categoria
                      ) in permisosAgrupados"
                      :key="categoria"
                    >
                      <tr>
                        <th
                          colspan="2"
                          class="blue text-center lighten-3 black--text text--darken-2 font-weight-bold"
                        >
                          {{ categoria }}
                        </th>
                      </tr>
                      ...
                      <tr
                        v-for="(permiso, index) in permisosCategoria"
                      ...
                      >
                        ...
                        <td>
                          <v-switch
                            ...
                            :disabled="
                              index > 0 &&
                              !permisosAsignados.includes(
                                permisosCategoria[0].id_permiso
                              ) &&
                              categoria !== 'Reporte' &&
                              categoria !== 'Recibo'
                            "
  ...
  methods: {
    ...
    obtenerRol() {
      this.$api({
        ...
        const permisos = response.data.rol.permisos.map(
    ...
    updateSwitches(categoria) {
      ...
      if (!this.permisosAsignados.includes(primerPermiso)) {
        for (let i = 1; i < this.permisosAgrupados[categoria].length; i++) {
          const index = this.permisosAsignados.indexOf(
            this.permisosAgrupados[categoria][i].id_permiso
          );
          if (index !== -1) {
